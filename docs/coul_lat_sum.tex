...
basis $\mathbf{b}$
\begin{equation}
  \mathbf{k} = \mathbf{b}_1 k_1 + \mathbf{b}_2 k_2 + \mathbf{b}_3 k_3
  = k_x \mathbf{e}_x + k_y \mathbf{e}_y + k_z \mathbf{e}_z
  = (k_x, k_y, k_z)
  \label{}
\end{equation}
If the reciprocal basis is non-orthogonal,
the denominator of \eqref{eq:poisson} is calculated via
\begin{equation}
...
\begin{itemize}
  ...
  \item input: \verb$den_mat$ is the density matrix with dims nAO $\times$ nAO
    $\times$ nUnitCells
...
\item \verb$void init(const FSolidModel& solid, const FOpMatrix& den_mat);$
\item \verb$void cls::del();$
...
\item 
\begin{verbatim}
void cls::cells_accumulator((*cells_iter)(), (*loop_terminater)(),
        const int *grid_id, double *v, double *mat, double *ao);
\end{verbatim}
\begin{itemize}
...
\item 
\begin{verbatim}
void cls::grids_accumulator((*grids_iter)(),
