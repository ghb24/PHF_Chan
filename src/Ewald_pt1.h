...
class Ewald_pt1{
   ...
   public:
      //Ewald::Ewald takes in info about the unit cell: the Ewald parameter eta, number of nuclear charges in the supercell, the nuclear charges, the supercell translation vectors (T1 , T2 , T3 ) (3 vectors in cart co), locations of the nuclear charges within the supercell (cart co).
      Ewald_pt1(double eta); //FIXME

      virtual ~Ewald_pt1();

      //Ewald::NN calculates and returns the nuclear-nuclear repulsion energy W.
      double NN();
   ...
   private:
      ...
      //! The translation vectors defined in a right-handed cartesian coordinate system [size: 3*3]: (\vec{T}_i)_{j-coordinate in cart co} = dTvecs[i + 3*j]
      double * dTvecs;

      //! The reciprocal vector (right-handed cart co) [size: 3*3]: ( \vec{G}_i )_{j-co in cart co} = dGvecs[i + 3*j]
      double * dGvecs;
      ...
      //Calculate the signed volume (the triple product without taking the absolute value), the dTvecs should be initialized though
      double CalcSignedVolume();

      //Calculate the reciprocal lattive vectors in cart co, dTvecs and Volume should be OK then and dGvecs initialized.
      void CalcReciprocal();
