#!/usr/bin/python
'''Produce a makefile for compiling the phf source code for a specified target/configuration.
...
# pre-processing.
CPP = %(cpp)s
CPPFLAGS = -DMAXMEM='$(MAXMEM)' -D_VCS_VER='$(VCS_VERSION)' $(WORKING_DIR_CHANGES) -D_CONFIG='"$(CONFIG).($(OPT))"' %(cppflags)s
...
# config directories.
GDEP_DEST = dest/depend

# Directory for compiled executable.
EXE = bin

# Directory for phf libraries.
LIB = lib
...
# Main programs
PROGS = $(addprefix $(EXE)/,phf.x kphf.x)
...
# the most recently compiled one to (e.g.) phf.x or gphf-cpmd.a. 
...
# Compiling *.x
$(EXE)/phf.$(CONFIG).$(OPT).x: $(OBJECTS_PHF)
\t$(GBLD_ENV)
...
# This requires the JSS modified version of sfmakepend (supplied with phf).
...
\t@echo
\t@echo "Targets:"
\t@echo "phf.x        compile phf.x."
\t@echo "new           run clean and then compile phf.x."
...
MAKE_C_GDEPS = $(CCD) $(CPPFLAGS) $(CFLAGS) -MM -MT \$$\(GDEST\)/$(addsuffix .o,$(basename $(notdir $@))) $< -o $@
